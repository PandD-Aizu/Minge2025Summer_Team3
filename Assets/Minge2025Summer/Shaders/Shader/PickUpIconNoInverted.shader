Shader "Custom/PickUpIconNoInverted"
{
    Properties
    {
        [PerRendererData]_MainTex ("Sprite Texture", 2D) = "white" {}
        [PerRendererData]_RendererColor ("Renderer Color", Color) = (1,1,1,1)
        [PerRendererData]_Flip ("Flip", Vector) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings Vert (Attributes IN)
            {
                ...
                // 1. SpriteRendererのFlip設定
                float2 scale_flip = flip.xy;
                float2 offs_flip  = flip.zw;
                
                // 2. 形状計算用のUV (Flip設定を適用)
                float2 uv_shape = IN.uv * scale_flip + offs_flip;
                
                // 3. クアッドの形状を計算
                float2 quad;
                quad.x = uv_shape.x * 2.0 - 1.0;
                
                // --- 修正ロジック ---
                // STのYがマイナス（アトラスで反転）の場合、
                // uv_shape.y (1~0) と OUT.uv (0~1) のマッピングが逆転し反転する。
                // これを防ぐため、ST.y がマイナスの時だけ quad.y の計算を反転させる。
                if (stScale.y < 0)
                {
                    // uv_shape.y (1~0) を (0~1) に反転させてから (-1~1) に変換
                    quad.y = (1.0 - uv_shape.y) * 2.0 - 1.0; 
                }
                else
                {
                    // uv_shape.y (0~1) を (-1~1) に変換
                    quad.y = uv_shape.y * 2.0 - 1.0; 
                }
                // --- 修正ロジックここまで ---
                
                // 4. サンプリング用UVを計算 (形状UVにSTを適用)
                OUT.uv    = uv_shape * stScale + stOffset;
